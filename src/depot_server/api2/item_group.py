...
from depot_server.logic.item_group import item_group_from_orm
from .models.item_group import ItemGroup, ItemGroupPending
...
async def create_item_group(item_group: ItemGroupPending) -> ItemGroup:
...
async def update_item_group(item_group_id: UUID, item_group: ItemGroupPending) -> ItemGroup:
    """Update an existing item_group"""
    db_item_group: DbItemGroup = await ItemGroupRepo.update_item_group(item_group_id, **item_group.model_dump())
    return item_group_from_orm(db_item_group)
